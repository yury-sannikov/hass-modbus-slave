...
from custom_components.modbus_slave.bit_sensor import ModbusReadCache, setup_bit_sensors
from custom_components.modbus_slave.const import CONF_BIT_NUMBER, CONF_BIT_SENSORS
from custom_components.modbus_slave.modbus import ModbusHub
...
@mock.patch("custom_components.modbus_slave.bit_sensor._LOGGER")
...
async def test_register_sensor_last_state(
    mock_read, hass, read_data, last_state, expected, hass_storage
):
    """Run test for given config."""
    mock_read.side_effect = read_data
    switch_name = "modbus_test_switch"

    entity_id = f"{SENSOR_DOMAIN}.{switch_name}"
    hass_storage["core.restore_state"] = {
        "version": 1,
        "data": [
            {
                "state": {"state": last_state, "entity_id": entity_id},
